...
// Файлы, которые вообще может просить фронтенд. Список закрытый: имя
// файла приходит из запроса, и без проверки в него можно было бы
// подставить путь наружу хранилища.
...
// русскоязычного приложения это не край, а первый же случай. С прежним
// [a-z0-9-] такая коллекция не сохранялась вообще: запись падала на
// «Неизвестный файл», а картинки уезжали в папку, которую сервер потом
// отказывался отдавать. На сайте это работало — там файл лежал на
// GitHub, где кириллица в имени допустима.
//
// Запрещено ровно то, что делает имя опасным или незаписываемым:
// разделители пути, точка (она отделяет расширение) и знаки, которые
// Windows не пускает в имя файла. Дефис разрешён — slugify ставит его
// и между словами, и перед хвостом-суффиксом.

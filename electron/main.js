...
async function createWindow({ compact = false } = {}) {
  ...
  // Страховка: если "ready-to-show" почему-то не пришло вовсе, окно
  // так и осталось бы невидимым навсегда – show:false выше не включает
  // его само. Не редкий случай "иногда чуть задержалось": на одной
  // машине (Windows, антивирус ни при чём – проверено исключением)
  // ready-to-show не приходило вообще, сколько ни жди, а с этой
  // страховкой всё работает нормально после принудительного show().
  // Похоже на известную особенность Chromium/Electron – на некоторых
  // связках видеодрайвера ready-to-show не срабатывает для окна,
  // которое ещё ни разу не было видимым (композитор не рисует то, что
  // не показывают), и вырваться из этого может только сам show().
  // 500мс – почти не заметно на тех машинах, где ready-to-show и так
  // приходит быстро (там она в принципе не успевает выстрелить первой),
  // и оно же на глаз "мгновенно" там, где ready-to-show зависает
  // насмерть – раньше здесь стояли 8 секунд ради перестраховки, но раз
  // включение принудительно всё равно единственный путь на такой
  // машине, ждать так долго незачем.
  setTimeout(() => {
    if (!shown && !created.isDestroyed()) created.show();
  }, 500).unref();
  ...
  closingWin.on("close", (e) => {
    ...
    quitLog = [];
    e.preventDefault();
    // Прячем сразу – с точки зрения человека окно и так уже закрылось;
    // синхронизация (см. её же комментарий у finish() ниже) дорабатывает
    // в уже невидимом окне, а не заставляет ждать её честно на экране.
    // hide() не трогает сам рендерер – страница внутри продолжает
    // работать (и её executeJavaScript ниже) как ни в чём не бывало,
    // просто без видимого окна поверх неё; настоящее закрытие (close())
    // происходит только после finish().
    closingWin.hide();
    debugLog("close: спрятал окно, жду __syncBeforeQuit до 8с в фоне");
    ...
    // Потолок ожидания – теперь просто верхняя граница фоновой работы,
    // а не время, которое человек проводит перед закрывшимся на вид
    // окном (см. её же hide() выше): можно позволить себе не торопиться
    // ради надёжности синхронизации, а не резать его в первую очередь.
    const timeout = new Promise((resolve) => setTimeout(resolve, 8000)).then(() =>
      finish("timeout-8s")
    );

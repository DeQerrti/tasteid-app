...
async function createWindow({ compact = false } = {}) {
  ...
  // Страховка: если "ready-to-show" почему-то не пришло вовсе (первая
  // отрисовка зависла — например, антивирус или ещё не готовый сетевой
  // стек ОС на самом первом запуске придержали что-то внутри страницы),
  // окно так и осталось бы невидимым навсегда — show:false выше не
  // включает его само. Через 8 секунд показываем принудительно, даже
  // если внутри ещё не всё дорисовалось: пустой экран на секунду лучше,
  // чем зависшее "ничего не открывается" при живом процессе.
  setTimeout(() => {

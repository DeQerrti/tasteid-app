...
//
//  После перехода на SPA (фаза 4) этот файл подключает одна лишь
//  add.html — единственный оставшийся отдельный документ. Остальные
//  редакторы стали хэш-маршрутами внутри index.html, и там в «Историю
//  версий» ведёт обычная навигация оболочки, а не плавающая кнопка.
//  Поэтому прежней защиты «не показывать кнопку на самой странице
//  истории» (проверки location.pathname на backup-history) здесь
//  больше нет: страницы backup-history.html не существует, а хэш
//  #/backup-history в этот документ никогда не попадает.
// ══════════════════════════════════════════════
...
(function () {
  ...
  function injectButtons() {
    ...
    historyBtn.addEventListener("click", () => {
      ...
      // отсюда (add.html — отдельный документ) это всё равно полная
      // навигация, но landing теперь на index.html, который эту
      // историю версий понимает сам, без отдельного файла.

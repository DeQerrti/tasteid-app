...
//
//  Общий для двух мест: маршрута #/backup-history (открывается с
//  плавающей кнопки «История» на add.html) и панели «История версий»
//  маршрута #/settings-edit. Оба используют одни и те же id разметки
//  (#fileTabs, #content) – просто в первом случае они на весь вид, во
//  втором лежат внутри одной из панелей настроек.
...
(function () {
  ...
  async function pruneHistoryNow() {
    ...
    if (!days) {
      backupToast(i18n("Сначала выберите, версии старше какого срока чистить."), false);
      return;
    }
    try {

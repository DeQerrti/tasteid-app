...
//  пока не удалён — прямая ссылка на него, если где-то сохранилась,
//  продолжит работать; его удаление — фаза 4). Единственное отличие
  ...
  function render() {
    ...
    if (!titles.length) {
      content.innerHTML = `<div class="state-box">${i18n("Нет тайтлов с флагом «Любимое».")}<br>
        <span style="font-size:.85rem">${i18n("Отметь тайтлы в редакторе отзыва.")}</span></div>`;
      return;
    }
    ...
    content.innerHTML = `
      ...
      <p class="hint">${i18n("Перетащи карточки чтобы изменить порядок. Нажми «Сохранить» когда готово.")}</p>

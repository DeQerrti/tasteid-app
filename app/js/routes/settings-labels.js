...
async function saveSettings() {
  ...
  try {
    ...
    //
    // await обязателен: applyTheme() и есть то место, где
    // window.SITE_HIDDEN_STATS (и вся остальная кэшированная копия
    // настроек) переписывается свежими данными. Без ожидания
    // refreshOpenReviewsTab() ниже иногда успевал перерисовать
    // «Статистику» ДО того, как applyTheme() дочитает site-settings.json
    // с диска, – и блок, который только что скрыли, на секунду успевал
    // получить неверный ответ isStatsBlockVisible() и остаться видимым
    // до следующего переключения вкладки. Показ ранее скрытого блока
    // эту же гонку почти всегда выигрывал по чистой случайности (файл
    // settings меньше reviews и обычно читается быстрее), поэтому
    // раньше казалось, что ломается только скрытие, а не обе стороны.
    if (data.ok) await applyTheme();

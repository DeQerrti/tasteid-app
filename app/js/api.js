...
async function fetchReviews() {
  try {
    ...
    // Настоящий сбой чтения (повреждённый reviews.json, недоступный
    // сервер) снаружи неотличим от «отзывов правда нет» – весь код
    // ниже одинаково молча показал бы пустые экраны. Хотя бы в
    // консоли причина остаётся видна, а не теряется совсем.
    console.error("Не удалось прочитать reviews.json:", err);
    cache.reviews = [];
    return [];
  }
}

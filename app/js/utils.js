...
// В этом репозитории – только приложение (Electron/мобилка), сайта
// tasteid.ru здесь нет и никогда не будет: /api/app/info отвечает
// всегда, любой сервер тут – это core/api.js, поднятый из electron/
// или mobile/. isAppContext() был нужен, когда один и тот же фронтенд
// шёл и на сайт (там адреса не отвечали, ветка «сайт» была живой), и
// в приложение – теперь ветка всегда одна, спрашивать нечего.
...
async function deleteMediaFile(relPath) {
  ...
  // притащить его обратно как "картинку, которой тут никогда не было"
  // (см. её же подробный комментарий у deleteRemoteMedia в js/sync.js).
  // Сама по себе тихая – деталей о том, настроена ли синхронизация,
  // здесь знать незачем, deleteRemoteMedia сама решит, что делать.
  //
  // await, а не "запустили и забыли": раньше это было фоновым вызовом
  // без ожидания, и при удалении СРАЗУ НЕСКОЛЬКИХ файлов подряд (поиск
  // осиротевших обложек – deleteAllOrphanedCovers в backup-history.js)
  // ничто не мешало нажать «Синхронизировать сейчас» раньше, чем
  // фоновые запросы на удаление в репозитории успевали дойти до конца.
  // Синхронизация в этот момент видела файл ещё не удалённым в
  // репозитории (запрос ещё летит) и тут же тянула его обратно на
  // диск как новый – реальный случай: 5 удалённых сирот возвращались
  // по одной за раз при синхронизации сразу после удаления. Вызывающая
  // сторона, которой ожидание не нужно (простая смена одной обложки),
  // как и раньше просто не ставит перед этим await – поведение для неё
  // не меняется.
  await deleteRemoteMedia(relPath);
}
...
// ── «Есть несохранённые изменения» – общий диалог для любого маршрута ──
// Раньше у каждого места со своими несохранёнными правками (add.js,
// settings-edit.js) был свой вариант вопроса «Остаться / Уйти без
// сохранения», а кнопка «Остаться» просто отменяла уход, оставляя
// правки висеть несохранёнными до следующей попытки. У порядка
// (reviews-order.js, favorites-edit.js, chars-edit.js) такого вопроса
// не было вовсе – уйти можно было молча, потеряв перетасканный порядок.
//
// Здесь и то, и другое сразу: одна функция на любой маршрут (нынешний
// и будущий) – передать, дирижно ли сейчас, и как сохранить – и кнопка
// вместо «Остаться» реально сохраняет и сама уходит, а не просто гасит
// диалог. Строгий режим (см. confirmDialog) обязателен: если «Уйти без
// сохранения» стало настоящим действием, а не безопасным «остаться» по
// умолчанию, случайный клик мимо диалога/Escape не должен молча
// стирать правки.
//
// Возвращает true, если теперь можно продолжать уход (правок не было,
// сохранены, или человек осознанно отказался от них), false – если
// нужно остаться (сохранение запрошено, но не удалось – ошибку сам
// save() уже показал своим способом).
async function confirmLeaveIfDirty({ isDirty, save, message }) {
...
// этого крутился бесконечно. Каждый вызывающий перед этим сам
// принудительно ставит loading="eager" (см. его же комментарий в
// tierlist.js/favorites.js/stats.js) именно для того, чтобы вместо
// «никогда» получить «когда-нибудь» – так что зависание навсегда
// самим пределом уже не грозит, а вот его ДЛИНА имеет значение сама по
// себе: у большого тир-листа (сотни местных картинок разом) браузер
// держит не больше 6 одновременных соединений на источник, и остальные
// просто стоят в очереди – никакой ошибки, они рано или поздно
// догрузятся, но фиксированных 8 секунд на сотни картинок может не
// хватить. Раньше это давало ровно то же самое пустое место на
// снимке, что и настоящая нехватка данных, – только по количеству
// картинок, а не по их отсутствию: с фильтром на один тип (десятки
// карточек) снимок собирался нормально, а с "все" (сотни) – нет.
// Поэтому предел растёт вместе с количеством картинок, а не остаётся
// фиксированным.
// Общий первый шаг перед html2canvas – tierlist.js/favorites.js/
// stats.js звали эти же три строки порознь, каждый копией предыдущего.
// __mobileForceResolveImages – только на телефоне (mobile/src/main.js):
// свой ленивый мост там резолвит src лишь у карточек, реально попавших
// во вьюпорт, а снимок собирает список целиком, в том числе то, что
// ниже прокрутки, – без принудительного резолва там оставался бы
// "сырой" /chars/... адрес, который waitForImages ниже принял бы за
// уже загруженный (ошибка загрузки тоже даёт img.complete===true), и
// на снимке было бы пусто именно там, где список длиннее экрана. На
// компьютере такой глобальной функции нет вовсе – вызов просто не
// находит её.
async function forceLoadImagesForExport(imgs) {
  imgs.forEach((img) => {
    img.loading = "eager";
  });
  await window.__mobileForceResolveImages?.(imgs);
...
// ── Предел размера холста перед html2canvas ────
// GPU-ускоренный canvas ограничен максимальным размером текстуры
// видеокарты – часто 8192 или 16384 пикселей по одной стороне на
// компьютере, но заметно меньше на телефоне: у многих мобильных GPU
// (особенно недорогих/не самых новых) реальный потолок ближе к 4096.
// html2canvas в таком случае не падает с понятной ошибкой – он просто
// не дорисовывает содержимое за этой границей, и часть картинки
// выходит пустой, хотя все данные для неё были на месте. У большого
// тир-листа или годового дайджеста статистики (сотни карточек)
// итоговый холст на фиксированном scale:2 может запросто перешагнуть
// эту границу, а у того же самого списка, отфильтрованного до части
// карточек, – уложиться в неё без проблем. Вместо фиксированного
// масштаба – наибольший, при котором обе стороны остаются в
// безопасных пределах: для небольших списков это как и раньше 2, для
// очень больших – меньше, картинка чуть менее чёткая, но целиком, без
// пропусков. Достоверно узнать лимит конкретного устройства неоткуда –
// сама отрисовка не сообщает о нём никак, – поэтому на телефоне берём
// заведомо консервативный потолок вместо десктопного: 8000 на телефоне
// уже один раз оказался мал (верхние тиры срезало на реальном
// устройстве, хотя тот же список безопасно укладывался в песочнице без
// настоящей видеокарты).
function safeCaptureScale(node, desiredScale, maxDim) {
  const limit = maxDim ?? (window.Capacitor?.isNativePlatform?.() ? 4096 : 8000);
  const longestSide = Math.max(node.scrollWidth, node.scrollHeight) * desiredScale;
...
// ── Esc уходит со страницы – общее для видов-редакторов ──────
// add.html и маршруты-редакторы (#/settings-edit, #/backup-history и
// подобные) открываются только кликом из главной, и раньше единственным
...
// справочной панели в настройках (маршрут #/settings-edit) – чтобы правка

...
// видеокарты – часто 8192 или 16384 пикселей по одной стороне,
// по-разному у разных видеокарт и драйверов. html2canvas в таком
// случае не падает с понятной ошибкой – он просто не дорисовывает
// содержимое за этой границей, и часть картинки выходит пустой, хотя
// все данные для неё были на месте. У большого тир-листа или
// годового дайджеста статистики (сотни карточек) итоговый холст на
// фиксированном scale:2 может запросто перешагнуть эту границу, а у
// того же самого списка, отфильтрованного до части карточек, –
// уложиться в неё без проблем. Вместо фиксированного масштаба –
// наибольший, при котором обе стороны остаются в безопасных пределах:
// для небольших списков это как и раньше 2, для очень больших –
// меньше, картинка чуть менее чёткая, но целиком, без пропусков.
function safeCaptureScale(node, desiredScale, maxDim = 8000) {
  const longestSide = Math.max(node.scrollWidth, node.scrollHeight) * desiredScale;
  return longestSide > maxDim ? (desiredScale * maxDim) / longestSide : desiredScale;
}

...
function resolveImage(img, src) {
  vaultSrc(src)
    .then(async (real) => {
      ...
      // blob-адрес настоящему <img> на экране. Без этого браузер иногда
      // красит место под картинкой пустым (тот самый плоский тёмный
      // прямоугольник в плотной сетке тир-листа, который сам собой
      // проходит после тапа или паузы) – декодирование blob: URL не
      // мгновенное, а .src= не ждёт его конца сам по себе. Здесь оно уже
      // случилось на отдельном элементе, так что настоящему <img>
      // достаётся уже готовый к отрисовке битмап, без паузы между кадром
      // без картинки и кадром с ней.
      try {
        const probe = new Image();
        probe.src = real;
        await probe.decode();
      } catch {
        // Не получилось продекодировать заранее – не страшно, просто
        // достанется без подстраховки, как раньше.
      }
...
// месте экрана: прямо над нижней панелью вкладок. Обычным confirmDialog()

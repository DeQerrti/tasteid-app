#!/usr/bin/env node
// Внутренние ссылки вида href="/add" — только для Electron.
//
// В electron/server.js такой адрес разворачивает сам сервер (пробует
// /add, потом /add.html). На телефоне сервера нет: Capacitor отдаёт
// файлы из app/ по буквальному имени, и там add.html, а не add.
...
// считаются рабочими, потому что для Electron это правда так.

...
console.log("Кнопка «Скопировать»");
// Без разрешения на буфер обмена (обычный случай на телефоне, где
// разрешение никто явно не давал) navigator.clipboard.writeText()
// отклоняется, и обработчик идёт в запасной путь после await — именно
// тут e.currentTarget, если его не сохранить заранее, браузер уже
// успевает обнулить, и клик по кнопке роняет страницу вместо того,
// чтобы просто показать «Скопировано». Само падение ловит общий
// page.on("pageerror") выше и проваливает весь файл — здесь только
// ждём ожидаемый, "живой" исход.
await context.grantPermissions([]); // явно без clipboard-write — как на телефоне, где никто его не выдавал
const failuresBefore = failures.length;
await page.click("#pp-share-copy");
await page.waitForFunction(
  () => document.getElementById("pp-share-copy")?.textContent === "Скопировано",
  null,
  { timeout: 3000 }
);
ok(failures.length === failuresBefore, "клик по «Скопировать» не роняет страницу");

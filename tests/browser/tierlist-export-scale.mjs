...
// проблем. Первая версия объяснения была про предел размера GPU-
// текстуры (см. всё ещё актуальный safeCaptureScale ниже – он не
// вредит и часть похожих случаев правда закрывает) – но она не
// объясняла главного: пустой была именно ВЕРХНЯЯ часть, а не какая
// попало, и именно она загружается быстрее всего, будь дело в холсте.
// Настоящая причина оказалась в js/utils.js: waitForImages() ждёт,
// пока все <img> в узле догрузятся (img.complete), но не дольше
// фиксированных 8 секунд, а после сдаётся и пускает html2canvas
// дальше как есть. Все обложки — местные резервные копии на локальном
// сервере (см. cover-export.mjs, подтверждено владельцем), но браузер
// держит не больше 6 одновременных соединений на источник — при
// фильтре "все" (сотни карточек разом) очередь не успевает вся
// догрузиться за 8 секунд, и застрявшие в очереди попадают на снимок
// недогруженными, то есть пустыми. У отфильтрованного списка карточек
// в разы меньше — успевает без проблем. Исправлено: у waitForImages()
// потолок теперь растёт вместе с количеством картинок вместо
// фиксированных 8 секунд на любое их число.
//
